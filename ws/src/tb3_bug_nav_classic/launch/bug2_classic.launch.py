...
def generate_launch_description():
    ...
    def setup_spawn_and_node(context):
        # Resolve world path and set defaults (from this world):
        world_path = world.perform(context)
        # Defaults: START at user_spot_light_0, GOAL at user_spot_light_1
        ...
        try:
            ...
            def find_pose(light_name):
                ...
                if m:
                    vals = [float(v) for v in m.group(1).strip().split()]  # x y z R P Y
            ...
            if s: sx, sy = s
            if g: gx, gy = g
            print(f"[bug2_launch] START from user_spot_light_0: ({sx:.3f}, {sy:.3f})")
            print(f"[bug2_launch] GOAL  from user_spot_light_1: ({gx:.3f}, {gy:.3f})")
        except Exception as e:
            print(f"[bug2_launch] Failed to read world lights: {e}")
        ...
        node = Node(
            package='tb3_bug_nav_classic', executable='bug2_node', output='screen',
            parameters=[{
                'goal_x': gx,
                'goal_y': gy,
            }]
